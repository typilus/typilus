...
class TypeLatticeGenerator():
    ...
    def __init__(self, typing_rules_path: str, max_depth_size:int=2, max_list_size:int= 2):
        ...
        with open(typing_rules_path, 'r') as f:
            rules = json.load(f)
        # alias -> default name
        self.__aliases = {parse_type_annotation_node(k): parse_type_annotation_node(v) for k, v in rules["aliasing_rules"]}
        ...
        self.__direct_inheritance_rewriting = DirectInheritanceRewriting(
            self.__is_a_relationships,
            self.__non_generic_types
            )

        self.__rewrites_verbose_annotations = RewriteRuleVisitor([RemoveUnionWithAnys(), RemoveStandAlones(), RemoveRecursiveGenerics(), RemoveGenericWithAnys()])
        assert len(self.__ids_to_nodes) == len(set(repr(r) for r in self.__ids_to_nodes))

    def create_alias_replacement(self, imported_symbols: Dict[TypeAnnotationNode, TypeAnnotationNode]) -> AliasReplacementVisitor:
    ...
    def __all_reachable_from(self, type_idx: int) -> FrozenSet[int]:
        ...
        while len(to_visit) > 0:
            ...
            to_visit.extend((parent_type_idx for parent_type_idx in self.is_a_edges[next_type_idx] if parent_type_idx not in reachable))
            ...
            if was_rewritten:
                if not erasure_happened or len(self.__to_process) < 5000 or len(all_inherited_types_and_self) < 5:
                    for type_annotation in all_inherited_types_and_self:
                        type_annotation = type_annotation.accept_visitor(self.__max_depth_pruning_visitor, self.__max_annotation_depth)
    ...
    def canonicalize_annotation(self, annotation: TypeAnnotationNode, local_aliases: Dict[TypeAnnotationNode, TypeAnnotationNode]) -> Optional[TypeAnnotationNode]:
