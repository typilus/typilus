...
from typilus.model.model import Model
...
def make_run_id(arguments: Dict[str, Any]) -> str:
    ...
    if user_save_name is not None:
        user_save_name = user_save_name[:-len('.pkl')] if user_save_name.endswith(
            '.pkl') else user_save_name
        return "%s" % user_save_name
    else:
        user_save_name = arguments.get('--model', 'graph2annotation')
